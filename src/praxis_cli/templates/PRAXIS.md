...
## PR Automation (Codex 5.3)

Every PR is automatically reviewed by Codex 5.3 via GitHub Action. The review checks:
- Spec compliance (if PR is linked to a PRAXIS track)
- Verification.md rules (lint, format, type check, security)
- Code quality and simplification opportunities

The GitHub Action is generated by `bootstrap.sh` at `.github/workflows/praxis-pr-review.yml`.
